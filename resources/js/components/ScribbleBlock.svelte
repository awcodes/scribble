<script>
    ...
    const open = () => {
        pounce(node.attrs.type, { update: true, statePath: $getStatePath, ...node.attrs.values })
    }
...
<NodeViewWrapper>
    <div class="relative group bg-gray-900/5 dark:bg-white/5 rounded-md">
        <div class="transition cursor-pointer rounded-md overflow-hidden z-10 relative {selected ? 'ProseMirror-selectednode' : ''}" bind:this={wrapper}>
            {@html view}
        </div>
